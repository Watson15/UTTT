...
class attack_bot:
    """
    Attack bot tries completing any lines with 2 markers in a row.
    It doesn't account for the opponent's actions in any way.
    """
    ...
    def __init__(self, name: str = 'dasher'):
        self.name = name

    def move(self, board_dict: dict) -> tuple:
        """
        Currently implementing the 'complete an immediate line' heuristic
        """
    ...
    def get_line_completions(self, board_state: np.array, active_board: tuple) -> [tuple]:
        """
        ...
        if active_board == (-1, -1):  # Every board is active, loop through all
            for x in range(3):
                for y in range(3):
                    line_completing_moves += self.complete_line(board_state, (x, y))
        else:  # Specific active board
            line_completing_moves += self.complete_line(board_state, active_board)

        # Only want unique moves
        return [t for t in set(tuple(i) for i in line_completing_moves)]

    def complete_line(self, board: np.array, active_board: tuple) -> [tuple]:
        """
